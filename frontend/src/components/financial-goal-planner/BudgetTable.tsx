    ...
    <div className="bg-white rounded-lg shadow-md p-6">
            ...
            {displayData.map((year) => (
              ...
              >
                ...
                <td className="text-right py-3 px-2">
                  {formatCurrency(year.startingSalary)}
                </td>
                ...
                <td className="text-right py-3 px-2 font-semibold">
                  {formatCurrency(year.endingSalary)}
                </td>
                <td className="text-right py-3 px-2">
                  {formatCurrency(year.monthlyNeeds)}
                </td>
                <td className="text-right py-3 px-2">
                  {formatCurrency(year.monthlyWants)}
                </td>
                ...
                <td className="text-center py-3 px-2 text-xs">
                  {formatPercentage(year.needsPercentage)}
                </td>
                <td className="text-center py-3 px-2 text-xs">
                  {formatPercentage(year.wantsPercentage)}
                </td>
                <td className="text-center py-3 px-2 text-xs">
                  {formatPercentage(year.investmentsPercentage)}
                </td>
              </tr>
            ))}
          </tbody>
      ...
      <div className="mt-4 p-4 bg-blue-50 rounded-lg">
        <p className="text-sm text-blue-800">
          <strong>Note:</strong> Job changes are highlighted in yellow. The
          allocation percentages adjust over time as your income grows, with
          investments taking a larger share.
        </p>
      </div>
    </div>
  );
