    ...
    <div className="bg-gradient-to-r from-purple-500 to-pink-600 rounded-lg shadow-lg p-6 text-white">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="text-right">
            ...
            <p className="text-2xl font-bold mt-1">
              {goalsAchievable
                ? formatCrores(surplus)
                : formatCrores(shortfall)}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        ...
        <div className="p-4 bg-white/10 rounded-lg backdrop-blur">
          ...
          <p className="text-2xl font-bold mt-1">
            {formatCrores(totalAccumulatedWealth)}
          </p>
      ...
      <div className="space-y-3">
        ...
        {(["short", "medium", "long"] as const).map((category) => {
          ...
          return (
            <div
              key={category}
              className="p-3 bg-white/10 rounded-lg backdrop-blur"
            >
      ...
      {!goalsAchievable && (
        <div className="mt-6 p-4 bg-red-500/20 border border-red-300/30 rounded-lg">
          ...
          <p className="text-sm">
            To achieve all your goals, consider: increasing your starting
            salary, adjusting goal amounts, extending timelines, or increasing
            investment returns through higher-risk investments.
          </p>
        </div>
      )}
    </div>
